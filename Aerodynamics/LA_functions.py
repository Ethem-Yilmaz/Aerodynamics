# -*- coding: utf-8 -*-
"""
Created on Wed Oct 28 16:43:55 2020
...
def comb(num, el):
    ...
    return int(factorial(num)/(factorial(el)*factorial(num-el)))
...
def sumSquaredResiduals(a, b, x, y):
...
def best_single_predictor(data, y):
    ...
    n = len(data[0])
    i = 0
    #min_err = [max(i) for line in data for column in data if data[line][column]>i ]
    min_err = max(data)
    while type(min_err)==type(data):
        min_err = max(min_err)
    min_j, min_a, min_b = None, None, None
    for j in range(n):
        x = column(j, data)
        a, b = line(x, y)
        err = sumSquaredResiduals(a, b, x, y)
        if err < min_err:
            min_err = err
            min_j, min_a, min_b = j, a, b
    weights = [0.0] * n
    weights[min_j] = min_a
    return weights, min_b
...
def center(x):
  mu = mean(x)
  return [x[i]-mu for i in range (len(x))]
